...
local function fetchFile(name, codeext)
	...
	if not isfile(name..'.'..codeext) then
	    writefile(name..'.'..codeext, game:HttpGet(string.format('https://raw.githubusercontent.com/jewjew123/ALSPLOIT/main/IS/SHIT/USE/CATVAPE/OR/%s.%s', readfile('compiler/commit.txt'), url, codeext)))
